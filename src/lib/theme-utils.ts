/**
 * Theme Management Utilities for Tale Forge
 * Supports multiple theme variants with accessibility and user experience enhancements
 */
...
export const THEME_CONFIGS: Record<ThemeVariant, ThemeConfig> = {
  ...
  twilight: {
    ...
    primaryColors: {
      background: '240 20% 8%', // #0D0D1A - Lighter than midnight
      foreground: '0 0% 95%', // #F2F2F2 - Softer white
      surface: '240 20% 12%', // #131323 - Lighter surface
      surfaceElevated: '240 20% 16%', // #1A1A2E - More elevated
      surfaceOverlay: '240 20% 20%', // #212138 - Lighter overlay
    },
    brandColors: {
      primary: '39 85% 55%', // Softer amber
      primaryForeground: '0 0% 100%',
      primaryHover: '33 85% 55%',
      secondary: '51 85% 55%', // Softer gold
      secondaryForeground: '240 20% 8%',
      accent: '270 60% 60%', // Purple accent for variety
      accentForeground: '0 0% 100%',
    },
    glassMorphism: {
      dark: '0 0% 0% / 0.5',
  ...
  dawn: {
    ...
    primaryColors: {
      background: '35 15% 15%', // #211F1C - Warm dark background
      foreground: '35 5% 90%', // #E5E4E2 - Warm light text
      surface: '35 12% 20%', // #2B2926 - Warm surface
      surfaceElevated: '35 10% 25%', // #353230 - Elevated warm surface
      surfaceOverlay: '35 8% 30%', // #3F3C39 - Overlay warm surface
    },
    brandColors: {
      primary: '25 100% 60%', // Warm orange-amber
      primaryForeground: '35 15% 15%',
      primaryHover: '20 100% 55%',
      secondary: '45 100% 65%', // Warm gold
      secondaryForeground: '35 15% 15%',
      accent: '320 50% 65%', // Rose gold accent
 ...
 */
export function getContrastRatio(theme: ThemeVariant): number {
  // Return predefined contrast ratios for accessibility
  const ratios = {
    midnight: 21, // High contrast
    twilight: 12, // Medium-high contrast
    dawn: 8, // Medium contrast
  };
  return ratios[theme];
}

/**
 * Determine if a theme should use light text
 */
export function shouldUseLightText(theme: ThemeVariant): boolean {
  return theme !== 'dawn'; // Dawn uses darker text
}

/**
 * Get recommended theme based on time of day
 */
export function getRecommendedTheme(): ThemeVariant {
  const hour = new Date().getHours();

  if (hour >= 6 && hour < 12) {
    return 'dawn'; // Morning
  } else if (hour >= 12 && hour < 18) {
    return 'twilight'; // Afternoon/Evening
  } else {
...
export function enableThemeTransitions(): void {
  ...
  // Apply transition to commonly changing properties
  ...
  transitionProps.forEach(prop => {
    root.style.setProperty(`--transition-${prop}`, 'var(--theme-transition)');
  });
}
 ...
 */
export function getAccessibilityInfo(theme: ThemeVariant): {
  contrastRatio: number;
  isAACompliant: boolean;
  isAAACompliant: boolean;
  recommendedForLowVision: boolean;
} {
  const contrastRatio = getContrastRatio(theme);

  return {
    contrastRatio,
    isAACompliant: contrastRatio >= 4.5,
    isAAACompliant: contrastRatio >= 7,
    recommendedForLowVision: theme === 'midnight' || theme === 'twilight',
  };
}